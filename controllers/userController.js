...
const apiGatewayUrl =
  "https://yeeq44bp8h.execute-api.us-east-1.amazonaws.com/sync-users"; // Here's the API Gateway URL
...
const sendLoginCode = async (req, res) => {
  ...
  try {
    ...
    req.session.message =
      "We have sent a One-Time Password to your email. Check your email 'spam' or 'junk' folder if necessary.";
    ...
    if (
      ...
      const userRole =
        userGroups.Groups.length > 0
          ? userGroups.Groups[0].GroupName
          : "students"; // Default to 'students' if no group

      req.session.role = userRole; // Store role in session

      // Log the stored email and role for verification
      console.log(
        `verifyLoginCode - Email stored in session: ${req.session.email}`
      );
      console.log(
        `verifyLoginCode - Role stored in session: ${req.session.role}`
      );
...
const fetchUserData = async (email, role) => {
  console.log(
    `fetchUserData - Fetching user data for email: ${email}, role: ${role}`
  );
  const params = {
    TableName: "users",
    Key: {
...
const triggerLambdaFunction = async (req, res) => {
  try {
